...
sub main {
    ...
    while (<>) {
        ...
        if (/^\s*$/) {
            ...
            #print Dumper(\%space);
            my $dropped;
            $dropped = &relax(\%bricks, \%space);
            print "Relaxing dropped $dropped bricks\n";
...
#                    print "$brickid CAN NOT be disintegrated\n";
            ...
            print "Can disintegrate ".scalar(keys %dis)." bricks\n";
            # brick 700 supports a brick that is not supported by anything else supports 678
            #&writebab($brickid, \%space, \%bricks, \%dis);
            my @check = keys %bricks;
            my $maxc = -1;
            my $maxid = -1;
            my $sum = 0;
            foreach my $tid (@check) {
                next if defined $dis{$tid};
                my $save_bricks = dclone \%bricks;
                my $save_space = dclone \%space;
                &erase($tid, \%bricks, \%space);
                delete $bricks{$tid};
                my $c = &relax(\%bricks, \%space);
                $dis{$brickid} = $c;
                print "Brick $tid disturbed $c bricks\n";
                if ($c > $maxc) {
                    $maxc = $c;
                    $maxid = $tid;
                }
                $sum += $c;
                %bricks = %{ dclone $save_bricks };
                %space = %{ dclone $save_space };
            }
            print "Biggest drop came from brick $maxid with $maxc total\n";
            print "Sum=$sum\n";
            return;
        } else {
